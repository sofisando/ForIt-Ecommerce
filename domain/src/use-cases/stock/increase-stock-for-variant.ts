...
interface IncreaseStockForVariantDeps {
  stockService: StockService;
}

interface IncreaseStockForVariantPayload {
  variantId: string;
  branchId: string | null;
  amount: number;
}

export async function increaseStockForVariant(
  { stockService }: IncreaseStockForVariantDeps,
  { variantId, branchId, amount }: IncreaseStockForVariantPayload
): Promise<Stock | Error> {
